...
# Autosave every 255 steps
Events.onStepTaken += proc {
  $Trainer.autosave_steps = 0 if !$Trainer.autosave_steps
  $Trainer.autosave_steps = 0 if $PokemonSystem.autosave == 1
  $Trainer.autosave_steps += 1
  if $Trainer.autosave_steps >= 255 && !pbMapInterpreterRunning? && (!$game_player.pbTerrainTag.ice && $PokemonGlobal.sliding == false)
    echo("Autosaving...")
...
class PokemonSaveScreen
  ...
  def getSaveInfoBoxContents(slot)
    ...
    map_str = (temp_save_data[:player].name).to_s + "'s House" if temp_save_data[:map_factory].map.map_id == 3
...
#           "#{elapsed_str}"
    ...
    return "<ac><c3=209808,90F090>#{map_str}</c3></ac>"+ # green
           "#{player_str}"+
           "#{game_str}"+
           "#{elapsed_str}"
  end
