...
def pbSurf
  ...
  if (!pbCheckHiddenMoveBadge(Settings::BADGE_FOR_SURF,false) || !$PokemonBag.pbHasItem?(:TM97) || !movefinder) && !$DEBUG
    return false
  end
  if $PokemonSystem.hmprompt == 0
    if pbConfirmMessage(_INTL("The water is a deep blue...\nWould you like to surf on it?"))
      ...
      pbCueBGM(surfbgm,0.5) if surfbgm && $PokemonSystem.surfmusic==0
...
def pbSurfacing
  ...
  if pbConfirmMessage(_INTL("Light is filtering down from above. Would you like to use Dive?"))
    ...
    pbFadeOutIn {
       ...
       $scene.transfer_player(false)
       surfbgm = GameData::Metadata.get.surf_BGM
       if surfbgm && $PokemonSystem.surfmusic==0
         pbBGMPlay(surfbgm)
       else
