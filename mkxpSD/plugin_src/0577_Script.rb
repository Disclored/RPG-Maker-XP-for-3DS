      ...
      if GameData::Item.get(item).is_important?
        if @bag.pbIsRegistered?(item)
          pbDrawImagePositions(self.contents,[
             ["Graphics/Pictures/Bag Party/icon_register",rect.x+rect.width-48,rect.y+4,0,0,-1,32]
          ])
        elsif pbCanRegisterItem?(item)
          pbDrawImagePositions(self.contents,[
             ["Graphics/Pictures/Bag Party/icon_register",rect.x+rect.width-48,rect.y+4,0,32,-1,32]
          ])
        end
      else
      ...
      if !@pokemon.egg?
        ...
        if @pokemon.superShiny?
          pbDrawImagePositions(@overlaysprite.bitmap,[[
             "Graphics/Pictures/shinysuper",66,44,0,0,16,16]])
        elsif @pokemon.shiny?
          pbDrawImagePositions(@overlaysprite.bitmap,[[
             "Graphics/Pictures/shiny",66,44,0,0,16,16]])
        end
      end
...
class PokemonBag_Scene
  ITEMLISTBASECOLOR     = MessageConfig::LIGHT_TEXT_MAIN_COLOR
  ITEMLISTSHADOWCOLOR   = Color.new(136,136,136)
  ITEMTEXTBASECOLOR     = MessageConfig::LIGHT_TEXT_MAIN_COLOR
  ITEMTEXTSHADOWCOLOR   = MessageConfig::LIGHT_TEXT_SHADOW_COLOR
  POCKETNAMEBASECOLOR   = MessageConfig::LIGHT_TEXT_MAIN_COLOR
  POCKETNAMEOUTLINECOLOR= MessageConfig::LIGHT_TEXT_SHADOW_COLOR
  ITEMSVISIBLE          = 6
  ...
  def pbStartScene(bag,party,choosing=false,filterproc=nil,resetpocket=true)
    ...
    @sprites["machinetext"] = BitmapSprite.new(Graphics.width, Graphics.height, @viewport)
#    @sprites["machinetext"].z = @viewport.z - 1
  ...
  def pbRefreshIndexChanged
    ...
    # If selecting a TM, show stats.
    if itemlist.item != nil && GameData::Item.get(itemlist.item).is_machine? #TMs Pocket
      machine = GameData::Item.get(itemlist.item).move
      move = GameData::Move.get(machine)
      type = (move.type.name.to_s == "QMARKS") ? "???" : move.type.name.to_s
      moveCategory = ["Physical","Special","Status"]
      power = move.base_damage.to_s
      power = "-" if power == "0"
      power = "???" if power == "1"
      acc = (move.accuracy == 0) ? "-" : move.accuracy.to_s
      pp = move.total_pp.to_s

      bitmap = @sprites["machinetext"].bitmap
      bitmap.clear
      pbSetSmallFont(bitmap)
      @sprites["machinetext"].visible = true
      ...
      # oh ye and also show the actual move's description lol
      @sprites["itemtext"].text = move.real_description.to_s
    else
      @sprites["machinetext"].bitmap.clear
      @sprites["machinetext"].visible = false
    end
  end
  ...
  def pbUpdateAnnotation
    ...
    elsif @bag.lastpocket == 6 && item #TMs Pocket
      annotations=nil
      annotations=[]
      if itm.is_machine?
        machine = itm.move
        ...
        if itemwindow.sorting
          ...
          if Input.trigger?(Input::LEFT)
            ...
            if itemwindow.pocket!=newpocket
              ...
              thispocket = @bag.pockets[itemwindow.pocket]
              pitch = 100 + (2*newpocket)
              pbSEPlay("GUI bag pocket",100,pitch)
              @sorting_mode = 0
              pbRefresh
            end
          ...
          elsif Input.trigger?(Input::SPECIAL)   # Checking party / Register Key Item
            if pbCanRegisterItem?(itemwindow.item)
              if @bag.pbIsRegistered?(itemwindow.item)
                @bag.pbUnregisterItem(itemwindow.item)
              else
                @bag.pbRegisterItem(itemwindow.item)
              end
                pbSEPlay("GUI bag register")
                pbRefresh
            else
          ...
          elsif Input.trigger?(Input::ACTION)   # Start switching the selected item / Sort the items list
#            if !@choosing
...
#              end
#            end
            if itemwindow.index<thispocket.length && !Settings::BAG_POCKET_AUTO_SORT[itemwindow.pocket] && thispocket.length>1
              pbSEPlay("GUI bag sort")
              thispocket.sort! do |a, b|
                item_a = GameData::Item.get(a[0])
                item_b = GameData::Item.get(b[0])
                case @sorting_mode
                when 0
                  item_a.id_number <=> item_b.id_number   # sort by ID
                when 1
                  item_a.name <=> item_b.name             # sort by name
                end
              end
              @sorting_mode = (@sorting_mode + 1) % 2
              pbRefresh
            end
