...
class PokemonPartyHud < Component
  def startComponent(viewport)
    super(viewport)
    # Overlay stuff
    @sprites["overlay"] = BitmapSprite.new(Graphics.width/2,Graphics.height,@viewport)
    ...
    @sshinybmp = Bitmap.new(MENU_FILE_PATH + "overlayShinySuper")
  end
  ...
  def refresh
    ...
    for i in 0...$Trainer.party.length
      ...
      next if !pokemon
      newLine = i>2 ? 1 : 0
      spacing = (Graphics.width/8) * (i - 3*newLine)
      # Pokémon Icon
      @sprites["pokemon#{i}"] = PokemonIconSprite.new(pokemon,@viewport) if !@sprites["pokemon#{i}"]
      @sprites["pokemon#{i}"].x = spacing
      @sprites["pokemon#{i}"].y = Graphics.height - (244 - 80*newLine)
      @sprites["pokemon#{i}"].z = -2
      next if pokemon.egg?
      # Information Overlay
      @sprites["overlay"].bitmap.blt(spacing + 16, Graphics.height - (182 - 80*newLine),
                          @infobmp, Rect.new(0, 0, @infobmp.width, @infobmp.height))
      # Shiny Icon
      if pokemon.superShiny?
        @sprites["overlay"].bitmap.blt(spacing + 52, Graphics.height - (222 - 80*newLine),
                          @sshinybmp,Rect.new(0, 0, @shinybmp.width, @shinybmp.height))
      elsif pokemon.shiny?
        @sprites["overlay"].bitmap.blt(spacing + 52, Graphics.height - (222 - 80*newLine),
      ...
      # Item Icon
      if pokemon.hasItem?
        @sprites["overlay"].bitmap.blt(spacing + 52, Graphics.height - (196 - 80*newLine),
      ...
      if pokemon.hp>0
        ...
        @sprites["overlay"].bitmap.blt(spacing + 18, Graphics.height - (180 - 80*newLine), @hpbar.bitmap, hprect)
      end
      # EXP
      if pokemon.exp>0
        ...
        @sprites["overlay"].bitmap.blt(spacing + 22, Graphics.height - (174 - 80*newLine), @expbar.bitmap, exprect)
      end
      # Status
      status = 0
      if pokemon.fainted?
      ...
      if status >= 0
        ...
        @sprites["overlay"].bitmap.blt(spacing + 48, Graphics.height - (186 - 80*newLine), @status.bitmap, statusrect)
      end
    end
  end
...
class DateAndTimeHud < Component
  ...
  def startComponent(viewport)
    super(viewport)
    @sprites["overlay"] = BitmapSprite.new(Graphics.width,152,viewport)
    @sprites["overlay"].ox = @sprites["overlay"].bitmap.width
    @sprites["overlay"].x = Graphics.width
    @baseColor = MENU_TEXTCOLOR[$PokemonSystem.current_menu_theme] || Color.new(248,248,248)
    @shadowColor = MENU_TEXTOUTLINE[$PokemonSystem.current_menu_theme] || Color.new(48,48,48)
  ...
  def refresh
    ...
    text3 = _INTL("${1}",$Trainer.money.to_s_formatted)
    spacing = 32
    @sprites["overlay"].bitmap.clear
    pbSetSystemFont(@sprites["overlay"].bitmap)
    pbDrawTextPositions(@sprites["overlay"].bitmap,[[text,8, 16 + spacing,0,
      @baseColor,@shadowColor],[text2,8,16 + spacing * 2,0,@baseColor,@shadowColor],[text3,8,16 + spacing * 3,0,@baseColor,@shadowColor]])
    @last_time = pbGetTimeNow.strftime("%I:%M %p")
  end
end
...
class NewQuestHud < Component
  ...
  def refresh
    ...
    @sprites["overlay"].bitmap.clear
    if numQuests > 0
      text = _INTL("You have {1} new task{2}!",numQuests, numQuests == 1 ? "" : "s")
