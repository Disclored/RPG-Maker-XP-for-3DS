class Sprite_Character
  ...
  DUPLICATE_FOOTSTEPS_WITH_FOLLOWER = true
    ...
    if (@character.x != @old_x || @character.y != @old_y) && !["", "nil"].include?(@character.character_name) && $PokemonSystem.performance==0
      if @character == $game_player && $PokemonTemp.dependentEvents &&
        ...
        make_steps = [2,1,0].any? do |e|
          ...
          next $data_tilesets[tilesetid].terrain_tags[tile_id] == 3 || $data_tilesets[tilesetid].terrain_tags[tile_id] == 20 #PBTerrain::Sand/Snow
        end
      end
      if make_steps
