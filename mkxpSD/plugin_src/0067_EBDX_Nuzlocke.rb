...
#    @nuzlocke = force.nil? ? !@nuzlocke : force
    ...
    $PokemonGlobal.isNuzlocke = force.nil? ? !$PokemonGlobal.isNuzlocke : force
  end
  #-----------------------------------------------------------------------------
  #  check if perma-faint is on
  #-----------------------------------------------------------------------------
  def self.permaFaint?
#    data = EliteBattle.get_data(:NUZLOCKE, :Metrics, :RULES); data = [] if data.nil?
#    return EliteBattle.get(:nuzlocke) && (data.include?(:NOREVIVE) || data.include?(:PERMADEATH))
    data = ($PokemonGlobal&.nuzlockeRules || [])
    return EliteBattle.nuzlocke? && (data.include?(:NOREVIVE) || data.include?(:PERMADEATH))
  end
  #-----------------------------------------------------------------------------
...
#        if EliteBattle.get(:nuzlocke) && data.include?(:ONEROUTE) && battler.index%2 == 1
        if EliteBattle.nuzlocke? && data.include?(:ONEROUTE) && battler.index%2 == 1
...
#    if EliteBattle.get(:nuzlocke) && data.include?(:PERMADEATH)
    data = ($PokemonGlobal&.nuzlockeRules || [])
    if EliteBattle.nuzlocke? && data.include?(:PERMADEATH)
...
#    if EliteBattle.get(:nuzlocke) && data.include?(:ONEROUTE)
    data = ($PokemonGlobal&.nuzlockeRules || [])
    if EliteBattle.nuzlocke? && data.include?(:ONEROUTE)
      static = data.include?(:STATIC) && !$nonStaticEncounter
...
#    if EliteBattle.get(:nuzlocke) && data.include?(:ONEROUTE) && @decision == 4
    if EliteBattle.nuzlocke? && data.include?(:ONEROUTE) && @decision == 4
...
#    if EliteBattle.get(:nuzlocke) && data.include?(:ONEROUTE) && !self.opponent
    if EliteBattle.nuzlocke? && data.include?(:ONEROUTE) && !self.opponent
...
#  if EliteBattle.get(:nuzlocke)
  if EliteBattle.nuzlocke?
    pbMessage(_INTL("\\w[]\\wm\\c[12]\\l[3]All your Pokémon have fainted. You have lost the Nuzlocke challenge! The challenge will now be turned off."))
...
#    end
    if $PokemonGlobal
      EliteBattle.set(:nuzlocke, $PokemonGlobal.isNuzlocke)
      EliteBattle.add_data(:NUZLOCKE, :RULES, $PokemonGlobal.nuzlockeRules || [])
    end
    return ret
  end
end
