...
#    @randomizer = force.nil? ? !@randomizer : force
    ...
    $PokemonGlobal.isRandomizer = force.nil? ? !$PokemonGlobal.isRandomizer : force
...
#    for key in EliteBattle.get_data(:RANDOMIZER, :Metrics, :RULES)
    for key in ($PokemonGlobal&.randomizerRules || [])
...
#    added = EliteBattle.get_data(:RANDOMIZER, :Metrics, :RULES)
    added = ($PokemonGlobal&.randomizerRules || [])
...
#  return species if !EliteBattle.get(:randomizer)
  return species if !EliteBattle.randomizer?
  pokemon = nil
...
#  return item if !EliteBattle.get(:randomizer)
  return item if !EliteBattle.randomizer?
