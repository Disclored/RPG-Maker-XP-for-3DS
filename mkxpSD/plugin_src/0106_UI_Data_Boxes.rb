...
class DataBoxEBDX  <  SpriteWrapper
  ...
  def applyMetrics
    ...
    # set XY positions
    @defX = @playerpoke ? @viewport.width - (tbmp.width + 2) : 0
    ...
    @data = {
      ...
      "container" => {:x => @playerpoke ? 22 : 26, :y => 6, :z => 1},
  ...
  def refresh
    ...
    str += " "
    color = Color.white
    color = Color.new(115,217,242) if @pokemon.shiny?
    color = Color.new(226,115,115) if @pokemon.superShiny?
