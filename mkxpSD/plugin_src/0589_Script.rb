...
class PokemonPokedexInfo_Scene
  ...
  def pbUpdate
    ...
    if Input.trigger?(Input::JUMPUP)
      if @page == 4
        ...
        displaySubPage
        pbSEPlay("se_theme_switch") if $Trainer.owned?(@species)
      end
  ...
  def drawPageAdvanced
    ...
    @sprites["infoverlay"].setBitmap(_INTL("Graphics/Pictures/Pokedex/overlay_advanced"))
...
#        )
  ...
  def displaySubPage
    ...
    shadow = Color.new(115,115,115)
    
    # Bottom text  
    textpos = [[
      "",Graphics.width/2,Graphics.height-124,2,base,shadow
    ]]
    if $Trainer.owned?(@species)
      textpos.push([
      ...
      textpos.push([
        _INTL("< [{1}]",Keys.key_name($PokemonSystem.game_control_code("Scroll Up")[0]) || "A"),
    ...
    textpos = []
    textpos.push([_INTL("Advancepedia"),58,0,0,base,shadow])
    textpos.push([_INTL("{1}",@data.name),
       Graphics.width/1.4,0,2,base,shadow])
    ...
    if(@data.type1==@data.type2)
      ...
      overlay.blt(typeBaseX-36,line2Y,@typebitmap.bitmap,type1rect)
      overlay.blt(typeBaseX+36,line2Y,@typebitmap.bitmap,type2rect)
    end
  ...
  def getInfo
    ...
    # Base Stats
    ret[0][0]=_ISPRINTF(
      "                             HP ATK DEF SAT SDF SPD Tot"
    )
    ret[0][1]=_ISPRINTF(
      "Base Stats:       {1:03d} {2:03d} {3:03d} {4:03d} {5:03d} {6:03d} {7:03d}",
      @data.base_stats[:HP],@data.base_stats[:ATTACK],@data.base_stats[:DEFENSE],
      @data.base_stats[:SPECIAL_ATTACK],@data.base_stats[:SPECIAL_DEFENSE],
      @data.base_stats[:SPEED],@data.base_stats.values.sum
    )
    # Effort Points
    ret[0][2]=_ISPRINTF(
      "EV Yield:             {1:03d} {2:03d} {3:03d} {4:03d} {5:03d} {6:03d} {7:03d}",
      @data.evs[:HP],@data.evs[:ATTACK],@data.evs[:DEFENSE],
      @data.evs[:SPECIAL_ATTACK],@data.evs[:SPECIAL_DEFENSE],
      @data.evs[:SPEED],@data.evs.values.sum
    )
    # Abilities
    ...
    ret[0][4]=_INTL("Hidden Ability: {1}", hiddenAbilityFirstString)
    ...
    # Base Exp
    ret[2][0]=_INTL("Base Exp Yield: {1}",@data.base_exp)
    # Catch Rate
    ...
    if genderRatio.female_chance
      genderString = _INTL("{1}% Male, {2}% Female",100-100*genderRatio.female_chance/256,100*genderRatio.female_chance/256)
    else
      genderString = genderRatio.name
    end
    ret[2][3]= _INTL("Gender Ratio: {1}", genderString)
    # Egg Steps to Hatch
    ret[2][4]= _INTL(
      "Hatch Time: {1} steps ({2} cycles)",
    ...
    ret[2][5]=_INTL("Egg Group: {1}",eggGroups)
    # Generation
    ret[4][0]=_INTL("Debut Gen: {1}",@data.generation) 
    # Happiness base
    ret[4][1]=_INTL("Base Friendship: {1}",@data.happiness)
    # Color
    ret[4][2]=_INTL("Colour: {1}",GameData::BodyColor.get(@data.color).name) 
    # Shape
...
#    end
    ...
    # Weaknesses
    ret[6][0]=_INTL("Weak: {1}",(getWeaknesses[0..[3, getWeaknesses.size].min]).join(", "))
    ret[6][1]=_INTL("{1}",(getWeaknesses[4..getWeaknesses.size]).join(", ")) if getWeaknesses.size > 4
    # Resistances
    ret[6][2]=_INTL("Resist: {1}",(getResistances[0..[3, getResistances.size].min]).join(", "))
    ret[6][3]=_INTL("{1}",(getResistances[4..getResistances.size]).join(", ")) if getResistances.size > 4
    # Immunities
    ret[6][4]=_INTL("Immune: {1}",(getImmunities[0..[3, getImmunities.size].min]).join(", "))
    ret[6][5]=_INTL("{1}",(getImmunities[4..getImmunities.size]).join(", ")) if getImmunities.size > 4
    ...
    ret[8][1] = holdItemsStrings[1] if holdItemsStrings.size>1
    ret[8][2] = holdItemsStrings[2] if holdItemsStrings.size>2
    # Evolutions
    evolutionsStrings = []
  ...
  def getEvolutionMessage(evolution, method, parameter)
    ...
    ret = case method
      ...
      when :HappinessDay
        _INTL("{1} when happy during day",evoName)
      when :HappinessNight
        _INTL("{1} when happy during night",evoName)
      when :Beauty
        _INTL("{1} when beauty is greater than {2}",evoName,parameter) 
      when :DayHoldItem
        _INTL("{1} holding {2} during day",evoName,GameData::Item.get(parameter).name)
      when :NightHoldItem
        _INTL("{1} holding {2} during night",evoName,GameData::Item.get(parameter).name)
      when :HasMove
        _INTL("{1} with move {2}",evoName,GameData::Move.get(parameter).name)
      ...
      when :Solgaleo
        if $game_switches[12]
          _INTL("{1} at level {2} during day", evoName,parameter)
        else
          _INTL("{1} under Solar Monarch", evoName)
        end
      when :Lunala
        if $game_switches[12]
          _INTL("{1} at level {2} during night", evoName,parameter)
        else
          _INTL("{1} under Solar Monarch", evoName)
        end
      else
  ...
  def getWeaknesses
    ...
    arr = []
    weak = []
    GameData::Type.each { |t| arr.push(Effectiveness.calculate(t.id,@data.type1,@data.type2)) if !t.pseudo_type }
    arr.each_with_index do |z, i|
      currType = @types[i]
      weak.push(GameData::Type.get(currType).name) if Effectiveness.super_effective?(z)
    end
   weak.push(_INTL("None")) if weak.size<1
   return weak
  end
  ...
  def getResistances
    ...
    arr.each_with_index do |z, i|
      currType = @types[i]
      resist.push(GameData::Type.get(currType).name) if Effectiveness.not_very_effective?(z)
    end
   resist.push(_INTL("None")) if resist.size<1
   return resist
  end
  ...
  def getImmunities
    ...
    arr.each_with_index do |z, i|
      currType = @types[i]
      immune.push(GameData::Type.get(currType).name) if Effectiveness.ineffective?(z)
    end
   immune.push(_INTL("None")) if immune.size<1
   return immune
  end
