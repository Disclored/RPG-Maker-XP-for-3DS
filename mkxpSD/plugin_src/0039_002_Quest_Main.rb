...
#        pbMessage("You have already failed this quest.")
    ...
    name = $quest_data.getName(quest)
    pbMessage(_INTL("\\me[{1}]\\l[3]<ac><c2=#{colorQuest("red")}>New task discovered!</c2>\n<c2=#{colorQuest("blue")}>{2}</c2>\nCheck the Pokénect for more details!\\wtnp[100]</ac>",QUEST_JINGLE,name))
  end
...
#        pbMessage("You have already failed this quest.")
    ...
    for i in 0...@active_quests.length
      if @active_quests[i].id == quest
        ...
        name = $quest_data.getName(@active_quests[i].id)
        @failed_quests.push(temp_quest)
        @active_quests.delete_at(i)
        found = true
        pbMessage(_INTL("\\me[{1}]\\l[3]<ac><c2=#{colorQuest("red")}>Task failed!</c2>\n<c2=#{colorQuest("blue")}>{2}</c2>\nYour task log has been updated.\\wtnp[100]</ac>",QUEST_FAIL,name))
        break
      end
    end
    if !found
