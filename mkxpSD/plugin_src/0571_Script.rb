...
module Keys
  ...
  def self.default_controls
    return [
      ...
      ControlConfig.new("Down", "K"),
      ControlConfig.new("Left", "Left"),
      ControlConfig.new("Left", "J"),
      ControlConfig.new("Right", "Right"),
      ControlConfig.new("Right", "L"),
      ControlConfig.new("Up", "Up"),
      ControlConfig.new("Up", "I"),
...
class Window_PokemonControls < Window_DrawableCommand
  ...
  def initialize(controls,x,y,width,height)
    ...
    @sel_base_error = Color.new(248,56,32)
    @sel_shadow_error = Color.new(224,152,144)

    @name_base_colorD = Color.new(168,184,184)
    @name_shadow_colorD = Color.new(88,88,80)
    @sel_base_colorD = Color.new(136,168,208)
    @sel_shadow_colorD = Color.new(24,112,216)
    @sel_base_errorD = Color.new(224,152,144)
    @sel_shadow_errorD = Color.new(248,56,32)
  ...
  def control_description(control_action)
    ...
    hash["Cancel"      ] = _INTL("Exit, cancel a choice or mode, and move at a different speed in the field.")
    hash["Menu"        ] = _INTL("Open the menu. Also has various functions depending on context.")
    hash["Scroll Up"   ] = _INTL("Advance quickly in menus. Toggle walking Pokémon.")
    hash["Scroll Down" ] = _INTL("Advance quickly in menus.")
    hash["Ready Menu"  ] = _INTL("Open Ready Menu, to use registered items and available field moves.")
    hash["Quick Save"  ] = _INTL("Saves the game in the slot that was last loaded or saved in.")
    hash["Speed-Up"    ] = _INTL("Switches between different game speeds.")
  ...
  def drawItem(index,count,rect)
    ...
    pbDrawShadowText(
      ...
      name,(@isDark) ? @name_base_colorD : @name_base_color,(@isDark) ? @name_shadow_colorD : @name_shadow_color
    )
#    self.contents.draw_text(rect.x,rect.y,width,rect.height,name)
    ...
    xpos = width+rect.x
    c = (@isDark) ? @sel_base_colorD : @sel_base_color
    cs = (@isDark) ? @sel_shadow_colorD : @sel_shadow_color
    if value == "None"
      c = (@isDark) ? @sel_base_errorD : @sel_base_error
      cs = (@isDark) ? @sel_shadow_errorD : @sel_shadow_error
    end
    pbDrawShadowText(
  ...
  def update
    ...
    if self.active && self.index<=@controls.length
      if Input.trigger?(Input::C)
        if on_default_index?
          if pbConfirmMessage(_INTL("Are you sure you want to reset all control binds to their default setting?"))
#            pbPlayDecisionSE()
...
class PokemonControls_Scene
  def start_scene
    ...
    @isDark = isDarkWindowskin(@sprites["textbox"].windowskin)
    ...
    @isDark = isDarkWindowskin(@sprites["controlwindow"].windowskin)
        ...
        if Input.trigger?(Input::B) || (
          ...
          if(
            ...
            if @sprites["controlwindow"].controls.find{|c| c.key_code == 0}
              pbPlayBuzzerSE()
              @sprites["textbox"].text=_INTL("There are empty binds! Please fill them in before exiting!")
              should_refresh_text = false
            else
