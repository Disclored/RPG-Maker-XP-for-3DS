    ...
    if @oldindex != @index
      ...
      if @showTypeAdvantage && !(@battle.doublebattle? || @battle.triplebattle?)
        ...
        if @opponent.effects[PBEffects::Illusion]
          illusion = PokeBattle_Battler.new(@battle, target.index)
          illusion.pbInitialize(@opponent.effects[PBEffects::Illusion], target.index)
          target = illusion
        end
        @modifier = move.pbCalcTypeMod(moveType, user, target)
      end
      @oldindex = @index
    end
    for i in 0...@nummoves
