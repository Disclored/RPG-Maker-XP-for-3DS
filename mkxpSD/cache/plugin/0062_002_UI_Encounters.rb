...
class EncounterList_Scene
  ...
  def pbStartScene
    ...
    @sprites["locwindow"].y = (Graphics.height - @sprites["locwindow"].height)/2 + 2
    @sprites["locwindow"].windowskin = nil
    @sprites["locwindow"].baseColor = MessageConfig::DARK_TEXT_MAIN_COLOR
    @sprites["locwindow"].shadowColor = MessageConfig::DARK_TEXT_SHADOW_COLOR
    @sprites["overlay"] = IconSprite.new(0,0,@viewport)
    @sprites["overlay"].setBitmap("Graphics/Pictures/EncounterUI/overlay")
    @sprites["areaoutline"] = IconSprite.new(0,110,@viewport)
    @sprites["areaoutline"].setBitmap("Graphics/Pictures/EncounterUI/encounterBase")
    @sprites["area"] = IconSprite.new(0,112,@viewport)
    @sprites["area"].setBitmap("Graphics/Pictures/EncounterUI/base_land")
    @sprites["area"].visible = false
    ...
    @sprites["medal"] = IconSprite.new(414,28,@viewport)
    @sprites["medal"].setBitmap("Graphics/Pictures/EncounterUI/medal")
    @sprites["medal"].visible = false
  ...
  def drawPresent
    ...
    enc_array.each do |s|
      ...
      if !$Trainer.pokedex.owned?(s) && !seen_form_any_gender?(s,species_data.form)
        ...
        @sprites["icon_#{i}"].tone = Tone.new(-32,-32,-32,255)
        @sprites["icon_#{i}"].visible = true
        seenNum += 1
      else
        @sprites["icon_#{i}"].pbSetParams(s,0,species_data.form,false)
    ...
    if caughtNum >= enc_array.length
      @sprites["medal"].setBitmap("Graphics/Pictures/EncounterUI/medal_1")
      @sprites["medal"].visible = true
    elsif (caughtNum + seenNum) >= enc_array.length
      @sprites["medal"].setBitmap("Graphics/Pictures/EncounterUI/medal")
      @sprites["medal"].visible = true
    else
      @sprites["medal"].visible = false
    ...
    loctext = _INTL("<al>{1}: {2}</al>", $game_map.name,name)
    seenText = (seenNum+caughtNum).to_s + "/" + (enc_array.length).to_s
    caughtText = (caughtNum).to_s + "/" + (enc_array.length).to_s
    loctext += _INTL("<al>Seen: {1} | Caught: {2}</al>",seenText,caughtText)
#    loctext += sprintf("<c2=63184210>-----------------------------------------</c2>")
    @sprites["locwindow"].setText(loctext)
    drawAreaGraphic(currKey)
  end
  
  # Draw text if map has no encounters defined (e.g. in buildings)
  def drawAbsent
    loctext = _INTL("<al>{1}</al>", $game_map.name)
    loctext += sprintf("<al>This area has no encounters!</al>")
#    loctext += sprintf("<c2=63184210>-----------------------------------------</c2>")
    @sprites["locwindow"].setText(loctext)
  end

  def drawAreaGraphic(currKey)
    mapName = $game_map.name
    mapType = GameData::EncounterType.get(currKey).id.to_s
    name = mapType
    if mapType == "Land" || mapType == "Cave"
      if mapName == "Dimensional Forest" || mapName == "Route 5" || mapName == "Yoko Forest"
        name = "forest"
      elsif mapName == "Dragon Tomb"
        name = "ruins"
      elsif mapName == "Arcadia Ruins" && mapType != "Land"
        name = "arcadia"
      elsif mapName == "Malum Volcano"
        name = "lava"
      elsif mapName == "Alice Mansion"
        name = "mansion"
      elsif mapName == "Mount Laplaenta" && mapType == "Cave"
        name = "ice"
      elsif (mapName == "Mount Laplaenta" && mapType == "Land") || mapName == "Route 19"
        name = "snow"
      end
    end
    @sprites["area"].setBitmap("Graphics/Pictures/EncounterUI/base_"+name)
    @sprites["area"].visible = true
    @sprites["area"].opacity = 200
  end
