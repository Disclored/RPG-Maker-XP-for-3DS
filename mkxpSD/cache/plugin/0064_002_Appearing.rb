...
Events.FollowerRefresh += proc { |_pkmn|
  # The Pokemon disappears if the name of the map is 
  # Farnlor Gym or Beyond the Stars
  next false if [113,114,390].include?($game_map.map_id)
}
...
Events.FollowerRefresh += proc { |pkmn|
  # The Pokemon disappears if it is the following Pokemon:
  next false if pkmn.isSpecies?(:F00)
  next false if pkmn.isSpecies?(:MT)
  next false if pkmn.isSpecies?(:MT2)
  next false if pkmn.isSpecies?(:HUMANOID)
  next false if pkmn.isSpecies?(:MONSTER)
}
