...
class VoltseonsPauseMenu < Component
  def startComponent(viewport, spritehash, menu)
    ...
    @shouldRefresh = true
    path = MENU_FILE_PATH + "Backgrounds/"
    # Player Icon
    @sprites["player"] = Sprite.new(@viewport)
    if pbResolveBitmap(path + MENU_THEMES[$PokemonSystem.current_menu_theme][5] + "_#{$Trainer.character_ID}")
      @sprites["player"].bitmap = Bitmap.new(path + MENU_THEMES[$PokemonSystem.current_menu_theme][5] + "_#{$Trainer.character_ID}")
    else
      @sprites["player"].bitmap = Bitmap.new(path + MENU_THEMES[DEFAULT_MENU_THEME][5] + "_#{$Trainer.character_ID}")
    end
    @sprites["player"].z        = -1
    @sprites["player"].oy       = @sprites["player"].bitmap.height
    @sprites["player"].y        = @sprites["player"].bitmap.height
    # Hud image
    @sprites["hud"] = Sprite.new(@viewport)
    if pbResolveBitmap(path + MENU_THEMES[$PokemonSystem.current_menu_theme][3])
      @sprites["hud"].bitmap = Bitmap.new(path + MENU_THEMES[$PokemonSystem.current_menu_theme][3])
    else
      @sprites["hud"].bitmap = Bitmap.new(path + MENU_THEMES[DEFAULT_MENU_THEME][3])
    end
    @sprites["hud"].z        = -4
    @sprites["hud"].oy       = @sprites["hud"].bitmap.height
    @sprites["hud"].y        = @sprites["hud"].bitmap.height
    # Background image
    @sprites["menuback"] = Sprite.new(@viewport)
    if pbResolveBitmap(path + MENU_THEMES[$PokemonSystem.current_menu_theme][2])
      @sprites["menuback"].bitmap = Bitmap.new(path + MENU_THEMES[$PokemonSystem.current_menu_theme][2])
    else
      @sprites["menuback"].bitmap = Bitmap.new(path + MENU_THEMES[DEFAULT_MENU_THEME][2])
    end
    ...
    # Background Pattern
    @sprites["menupattern"] = Sprite.new(@viewport)
    @sprites["menupattern"].bitmap = Bitmap.new(MENU_FILE_PATH + "Backgrounds/bg")
    @sprites["menupattern"].z        = -6
    #@sprites["menupattern"].oy       = @sprites["menupattern"].bitmap.height
    #@sprites["menupattern"].y        = @sprites["menupattern"].bitmap.height
    ...
    @sprites["entrytext"].y = Graphics.height - 128
  ...
  def update
    ...
    if Input.trigger?(Input::BACK) || Input.trigger?(Input::ACTION)
      ...
      newVal = $PokemonSystem.current_menu_theme-1
      updateMenuTheme(newVal)
  ...
  def updateMenuTheme(newVal)
    path = MENU_FILE_PATH + "Backgrounds/"
    newVal = 0 if newVal > MENU_THEMES.length-1
    newVal = MENU_THEMES.length-1 if newVal < 0
    $PokemonSystem.current_menu_theme = newVal
    pbSEPlay("se_theme_switch")
    # Background Tone
    @sprites["backshade"].bitmap.fill_rect(0,0,Graphics.width,Graphics.height,BACKGROUND_TINT[$PokemonSystem.current_menu_theme])
    # Background Graphic
    @sprites["menuback"].bitmap.clear
    @sprites["menuback"].bitmap = Bitmap.new(path + MENU_THEMES[$PokemonSystem.current_menu_theme][2])
    # Hud Graphic
    @sprites["hud"].bitmap.clear
    @sprites["hud"].bitmap = Bitmap.new(path + MENU_THEMES[$PokemonSystem.current_menu_theme][3])
    # Trainer Graphic
    @sprites["player"].bitmap.clear
    @sprites["player"].bitmap = Bitmap.new(path + MENU_THEMES[$PokemonSystem.current_menu_theme][5] + "_#{$Trainer.character_ID}")
    # Arrow Graphics
    @sprites["rightarrow"].dispose
    @sprites["leftarrow"].dispose
    filename = path + MENU_THEMES[$PokemonSystem.current_menu_theme][0]
    ...
    filename = path + MENU_THEMES[$PokemonSystem.current_menu_theme][1]
  ...
  def refreshMenu
    ...
    middle = @displayIndexes.length/2
    for i in 0...@displayIndexes.length
      entry = @entries[@displayIndexes[i]]
      next if entry.nil?
