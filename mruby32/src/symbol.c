...
static mrb_sym
find_symbol(mrb_state *mrb, const char *name, size_t len, uint16_t *hashp)
{
  mrb_sym i;
  uint16_t hash;
  ...
  hash = (uint16_t)(mrb_byte_hash((const uint8_t*)name, len) & 0x3fff);  /* PATCH: 16384 baldes */
  if (hashp) *hashp = hash;
  ...
  for (;;) {
    ...
    uint32_t diff = mrb->symlink[i];   /* PATCH: era uint8_t com fallback O(N); uint32 = salto exato */
    if (diff == 0) return 0;
    i -= diff;
  }
}
...
sym_intern(mrb_state *mrb, const char *name, size_t len, mrb_bool lit)
{
  ...
  if (mrb->symcapa <= sym) {
    ...
    mrb->symlink = (uint32_t*)mrb_realloc(mrb, mrb->symlink, symcapa*sizeof(uint32_t));  /* PATCH: uint32 */
    mrb->symcapa = symcapa;
  }
  ...
  if (mrb->symhash[hash]) {
    mrb->symlink[sym] = sym - mrb->symhash[hash];  /* PATCH: salto exato (uint32), nunca overflow */
  }
  else {
    mrb->symlink[sym] = 0;
  }
